...
-- Linha do tempo de acoes por analise critica
-- critico_key = "{cod_frota}|{cod_compartimento}|{data_coleta}"
create table if not exists public.criticos_timeline (
  id bigint generated by default as identity primary key,
  critico_key text not null,
  texto text not null,
...
create index if not exists idx_criticos_timeline_key on public.criticos_timeline (critico_key);

alter table public.criticos_timeline enable row level security;

drop policy if exists "Usuarios logados podem consultar timeline criticos" on public.criticos_timeline;
create policy "Usuarios logados podem consultar timeline criticos"
on public.criticos_timeline for select to authenticated using (true);

drop policy if exists "Usuarios logados podem inserir timeline criticos" on public.criticos_timeline;
create policy "Usuarios logados podem inserir timeline criticos"
on public.criticos_timeline for insert to authenticated with check (true);

drop policy if exists "Usuarios logados podem excluir timeline criticos" on public.criticos_timeline;
create policy "Usuarios logados podem excluir timeline criticos"
on public.criticos_timeline for delete to authenticated using (true);
